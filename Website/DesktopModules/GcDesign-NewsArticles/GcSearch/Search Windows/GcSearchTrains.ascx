<%@ Control Language="C#" AutoEventWireup="false" CodeBehind="GcSearchTrains.ascx.cs" Inherits="GcDesign.NewsArticles.GcSearchTrains" %>
        ...
        $("#courses_data > a").on('click', function () {
            ...
            if ($(this).text() == $("#courses_data > a").first().text()) {
                ...
                if ($("#SearchConditionPane a[name='courses_data']") != undefined && $("#SearchConditionPane a[name='courses_data']").text() != $(this).text()) {
                    ...
                    element.on('click', element, function (e) { $(e.data).remove(); search_data.courses_data = 2; ajaxPost(); });
        ...
        $("#organization_data > a").on('click', function () {
            ...
            if ($(this).text() == $("#organization_data > a").first().text()) {
                ...
                if ($("#SearchConditionPane a[name='organization_data']") != undefined && $("#SearchConditionPane a[name='organization_data']").text() != $(this).text()) {
                    ...
                    element.on('click', element, function (e) { $(e.data).remove(); search_data.organization_data = 3; ajaxPost() });
                    $("#SearchConditionPane").append(element);
                }
            }
        ...
        $("#place_data > a").on('click', function () {
            ...
            if ($(this).text() == $("#place_data > a").first().text()) {
                ...
                if ($("#SearchConditionPane a[name='place_data']") != undefined && $("#SearchConditionPane a[name='place_data']").text() != $(this).text()) {
                    ...
                    element.on('click', element, function (e) { $(e.data).remove(); search_data.place_data = 6; ajaxPost(); });
                    $("#SearchConditionPane").append(element);
                }
            }
...
<div class="search_wrap">
    ...
    <div class="newsarticles">
        ...
        <asp:Repeater ID="rptLatestArticles" runat="server">
            ...
            <ItemTemplate>
                <li>
                    <a href='<%# GenerateArticleUrl(Container.DataItem) %>' target="_blank">
                        <p>
                            <img src='<%# GenerateImageUrl(Container.DataItem) %>' />
                        </p>
                        <div id="desWrap" runat="server"></div>
                        <p class="title">点击查看详情</p>
                    </a>
                </li>
            </ItemTemplate>
