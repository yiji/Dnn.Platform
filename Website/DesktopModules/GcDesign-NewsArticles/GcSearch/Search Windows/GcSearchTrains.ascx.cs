...
{
    public partial class GcSearchTrains : NewsArticleModuleBase
    {
        #region " Constants "
        ...
        private string courses_data = "2";
        private string organization_data = "3";
        private string place_data = "6";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (postback)
                {
                    string json = "$link$" + Common.GetModuleLink(_articleTabID, _articleModuleID, "Search", ArticleSettings, "courses_data=" + courses_data, "organization_data=" + organization_data, "time_data=" + time_data) + "$link$";
        ...
        private void BindListView()
        {
            ...
            List<CategoryInfo> lstCourses = controller.GetCategoriesAll(_articleModuleID, 2, CategorySortType.SortOrder);
            lvwCourses.DataSource = lstCourses;
            lvwCourses.DataBind();

            organization = controller.GetCategory(3, _articleModuleID);
            List<CategoryInfo> lstOrganization = controller.GetCategoriesAll(_articleModuleID, 3, CategorySortType.SortOrder);
            lvwOrganization.DataSource = lstOrganization;
            lvwOrganization.DataBind();

            place = controller.GetCategory(6, _articleModuleID);
            List<CategoryInfo> lstPlace = controller.GetCategoriesAll(_articleModuleID, 6, CategorySortType.SortOrder);
        ...
        private void BindArticles()
        {
            ...
            int[] categories = new int[] { int.Parse(courses_data), int.Parse(organization_data), int.Parse(place_data) };
            ...
            if (Session["Trains-" + TabModuleId.ToString()] == null || postback || (!IsPostBack && _pageNumber == 1))
            {
                //List<ArticleInfo> objArticles = objArticleController.GetSearchArticleList1(_articleModuleID, endDate,startDate , cats, matchAll,2, catsExclude, count, _pageNumber, pageSize, sort, sortDirection, true, false, Null.NullString, authorID, showPending, true, featuredOnly, notFeaturedOnly, securedOnly, notSecuredOnly, articleIDs, tagIDs, matchAllTags, Null.NullString, customFieldID, customValue, linkFilter, ref articleCount);
                objArticles = objArticleController.GetSearchArticleList1(_articleModuleID, endDate, startDate, cats, matchAll, 2, catsExclude, count, 1, 1000, sort, sortDirection, true, false, Null.NullString, authorID, showPending, true, featuredOnly, notFeaturedOnly, securedOnly, notSecuredOnly, articleIDs, tagIDs, matchAllTags, Null.NullString, customFieldID, customValue, linkFilter, ref articleCount);
                ...
                if (time_data == "year")
                {
                    ...
                    endTime = Convert.ToDateTime(DateTime.Now.AddYears(1).ToString("yyyy-MM-01 00:00:00"));
                }
                else
                {
                    DateTime time = Convert.ToDateTime(time_data);
                    startTime = Convert.ToDateTime(time.ToString("yyyy-MM-01 00:00:00"));
                    if (time.ToString("yyyy-MM") == DateTime.Now.ToString("yyyy-MM"))//当前月开始日期为当前日期
                    {
                        startTime = Convert.ToDateTime(DateTime.Now.ToString("yyyy-MM-dd 00:00:00"));
                    }
                    endTime = Convert.ToDateTime(time.AddMonths(1).ToString("yyyy-MM-01 00:00:00"));
                }



                newArticles = new List<ArticleInfo>();
                foreach (ArticleInfo article in objArticles)
                {
                    DateTime articleStartTime = article.StartDate;
                    DateTime articleEndTime = article.EndDate;
                    if (articleStartTime == null || articleEndTime == null)
                    {
                        continue;
                    }
                    int days = (articleEndTime - articleStartTime).Days;
                    for (int i = 0; i < days; i++)
                    {
                        if (articleStartTime.AddDays(i).CompareTo(startTime) > 0 && articleStartTime.AddDays(i).CompareTo(endTime) < 0)
                        {
                ...
                Session["Trains-" + TabModuleId.ToString()] = newArticles;
            }
            else
            {
                newArticles = (List<ArticleInfo>)Session["Trains-" + TabModuleId.ToString()];
            }


            //articleCount = newArticles.Count;

            //if (count < articleCount)
            //{
            //    articleCount = count;
            //}
            List<ArticleInfo> results = (from item in newArticles select item).Skip((_pageNumber - 1) * pageSize).Take(pageSize).ToList();
            ...
            int pageCount = ((newArticles.Count - 1) / pageSize) + 1;

            if ((newArticles.Count > 0) && pageCount > 1)
            {
                ...
                ctlPagingControl.TotalRecords = newArticles.Count;
        ...
        public void ArticleInfoDes(object article, ref Control control)
        {
            ...
            foreach (object obj in categories)
            {
                ...
                {
                    Literal course = new Literal();
                    course.Text = string.Format("<p>课程名：{0}</p>", category.Name);
                    controls.Add(1, course);

                }
                if (category.ParentID == 3)
                {
                    Literal organization = new Literal();
                    organization.Text = string.Format("<p>培训机构：{0}</p>", category.Name);
                    controls.Add(2, organization);

                }
                if (category.ParentID == 6)
                {
                    Literal place = new Literal();
                    place.Text = string.Format("<p>地点：{0}</p>", category.Name);
                    controls.Add(3, place);
